...
test(
  ...
  async ({ registerPage, accountPage, page, data }) => {
    ...
    // Hyvä prints "My Orders" twice — once as the <h1> page title and again as
    // an <h2 id="my-orders-title"> above the list — so the level has to be
    // pinned or the locator is ambiguous. Luma renders it once.
    await expect(
      page.getByRole('heading', { name: f.headingText, level: 1 }),
    ).toBeVisible();
    await expect(
