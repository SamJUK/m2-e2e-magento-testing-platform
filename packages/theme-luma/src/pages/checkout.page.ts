...
export class CheckoutPage implements ICheckoutPage {
  ...
  async submitOrder(): Promise<string> {
    ...
    // Match the confirmation line by text, not by heading role: Luma puts it in
    // an <h1>, but plenty of themes render it as a styled <div>/<span>, and
    // some put it outside the .checkout-success block entirely.
    await expect(
      this.page.getByText(f.success.headingText).first(),
    ).toBeVisible();

    // Guests get "Your order # is: <span>", logged-in customers get
    // "Your order number is: <a>" — one regex covers both wordings.
  ...
  private async selectShippingMethodAndPay(): Promise<void> {
    ...
    // Select shipping method. The rate radio carries no <label> — Magento
    // labels it with aria-labelledby pointing at the method and carrier cells,
    // which getByLabel does not resolve, so match the accessible name by role.
    // Some custom checkout KO templates strip that aria-labelledby and render
    // the rate as a styled row whose radio has no usable accessible name —
    // fall back to clicking the method's title text in that case.
    // NOT networkidle: real stores keep analytics/chat/search SaaS sockets
    // busy indefinitely, so networkidle never fires and burns the whole test
    // budget. Wait for the rate list itself instead.
    const shippingMethod = this.page
      .getByRole('radio', { name: s.shipping.methodFieldLabel })
      .first();
    const shippingMethodRow = this.page
      .getByText(s.shipping.methodFieldLabel, { exact: true })
      .first();
    await shippingMethod.or(shippingMethodRow).first().waitFor({ timeout: 30_000 });

    // Let the rate list settle before clicking it. Filling the address leaves
    // focus in the last field; the blur that a click delivers triggers an
    // estimate-shipping-methods refresh, and the Knockout `foreach` over the
    // rates replaces the row between mousedown and mouseup — so the click
    // event is dispatched onto a detached node and the binding never fires.
    // Blur first, out of band, so that refresh happens before the click.
    await this.page.evaluate(() => (document.activeElement as HTMLElement | null)?.blur());
    await this.page.locator('#checkout-loader').waitFor({ state: 'hidden' });

    const useNamedRadio = (await shippingMethod.count()) > 0;
    const clickTarget = useNamedRadio ? shippingMethod : shippingMethodRow;
    // Whichever target we click, verify against the rate's own radio input:
    // for the styled-row case that is the radio inside the row's nearest
    // radio-bearing ancestor.
    const rateRadio = useNamedRadio
      ? shippingMethod
      : shippingMethodRow
          .locator('xpath=ancestor::*[.//input[@type="radio"]][1]//input[@type="radio"]')
          .first();
    const verifiable = (await rateRadio.count()) > 0;

    // The rate rows re-render whenever Knockout refreshes the rate list, which
    // silently swallows the click and leaves the step unselected — the next
    // step then refuses to advance with "The shipping method is missing".
    // Retry until the rate actually sticks.
    // Short per-attempt verify: a click that landed applies within a second,
    // so a long wait here only burns the test budget on the attempts that were
    // swallowed. toPass keeps retrying either way.
    await expect(async () => {
      await clickTarget.click({ force: true });
      if (verifiable) await expect(rateRadio).toBeChecked({ timeout: 2_000 });
    }).toPass({ timeout: 60_000 });
    ...
    // rendered once that round-trip resolves. Wait on real signals rather than
    // a fixed pause: the URL fragment flipping to #payment, then the loader
    // clearing. Both are stock checkout behaviour, so neither is optional.
    await this.page.waitForURL(/#payment/, { timeout: 60_000 });
    await this.page.locator('#checkout-loader').waitFor({ state: 'hidden', timeout: 60_000 });
