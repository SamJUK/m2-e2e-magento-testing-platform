...
export class ProductPage implements IProductPage {
  ...
  constructor(page: Page, private data: MergedData) {
    this.page = page;
    const bundle = data.selectors.productPage.bundle;
    // The PDP's own final price, scoped away from the related/upsell tiles
    // further down the page. Read with readMoney (first VISIBLE match), so a
    // store rendering both inc- and exc-VAT copies compares like with like
    // against the minicart, cart and checkout figures.
    this.price = page.locator(data.selectors.productPage.priceSelector);
    ...
    const labels = ([] as string[]).concat(
      data.selectors.productPage.addToCartButtonLabel as string | string[],
    );
    const form = page.locator('#product_addtocart_form');
    this.addToCartButton = labels
      .map((name) => form.getByRole('button', { name, exact: true }))
  ...
  async addConfigurableProductToCart(
    ...
    const hasSwatches = (await this.page.locator('[data-role="swatch-options"]').count()) > 0;

    for (const [attrName, optionValue] of options) {
      if (hasSwatches) {
        await this.page.locator(`.swatch-option[aria-label="${optionValue}"]`).click();
      } else {
        await this.page.getByLabel(attrName).selectOption({ label: optionValue });
      }
    }
